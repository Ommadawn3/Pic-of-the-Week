...
export async function proxy(request: NextRequest) {
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
    // Supabase isn't configured yet (e.g. local UI work before Phase 4 auth
    // wiring, or before DEPLOYMENT_SETUP.md step 1 is done). Skip session
    // refresh rather than crashing every request.
    return response;
  }
